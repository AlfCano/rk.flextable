...
**📝 Smart Caption and Footer Management**

*   **Overwrite Controls:** Added new toggle options (checkboxes) to either overwrite existing table captions/footers or safely append new ones. 
*   **`gtsummary` Synergy:** You can now easily strip away unwanted automatic footnotes generated by `gtsummary` with a single click by leaving "Overwrite existing footer" checked.
*   **Footer Stacking & Positioning:** By unchecking the overwrite option, you can safely stack multiple lines in the footer. A new **Position dropdown** allows you to choose exactly where the new text goes: either at the **Bottom** (below existing notes) or at the **Top** (above them).
*   **Native Formatted Text (Unicode):** The footer text box natively supports Unicode characters! You can directly copy and paste superscripts (like `¹`, `²`) to write standard statistical notes (e.g., `¹n(%)`) without needing to write complex R formatting code.
